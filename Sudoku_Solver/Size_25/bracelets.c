...
void PushBack(BoardQueue* boards, Board* board) {
    boards->data = (Board**)realloc(boards->data, (boards->size + 1) * sizeof(Board*));
    boards->data[boards->size] = board;
    boards->size++;
...
void PartionSolve(BoardQueue* boards) {
    ...
    for (int num = 1; num <= BoardSize; num++) {
        if (ValidateBoard(fboard, x, y, num)) {
            ...
            for (int i = 0; i < BoardSize * BoardSize; i++) {
                newBoard->data[i] = fboard->data[i];
            }
            newBoard->data[x * BoardSize + y] = num;
            PushBack(boards, newBoard);
        }
    }
    PopFront(boards);
}
...
void DisorderQueue(BoardQueue* boards) {
    ...
    for (int i = 0; i < boards->size - 1; ++i) {
        ...
        Board* temp = boards->data[j];
        boards->data[j] = boards->data[i];
        boards->data[i] = temp;
    }
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < num_boards; i++) {
        ...
        problems[i]->size = 1;
        problems[i]->solved = 0;
        problems[i]->data = (Board**)malloc(sizeof(Board*));
